...
if MODEL not in {"ecmwf", "gfs", "icon"}:
    raise SystemExit("Uso: synoptic_700_full_phase66l.py ecmwf|gfs|icon")

# Reutilizamos el motor de descarga/proyección ya validado; cambiamos el nivel a 700 hPa.
...
# Estilo sinóptico específico de 700 hPa.
Z_BOUNDS = np.arange(1800.0, 4201.0, 30.0, dtype="float32")
TEMP_CONTOURS = np.arange(-52.0, 37.0, 4.0, dtype="float32")


def config():
    if MODEL == "ecmwf":
        return {
            "steps": ECMWF_STEPS, "model": "ECMWF IFS", "provider": "ECMWF Open Data",
            "getter": h.p66e._ecmwf_field, "display_bounds": h.GLOBAL_BOUNDS, "focus": h.FOCUS_GLOBAL,
            "intervals": [
                {"value": "auto", "label": "Automático (3/6 h)"}, {"value": "3", "label": "3 h"},
                {"value": "6", "label": "6 h"}, {"value": "12", "label": "12 h"}, {"value": "24", "label": "24 h"},
            ],
            "cadence": "+0…+144 cada 3 h; +150…+360 cada 6 h",
        }
    if MODEL == "gfs":
        return {
            "steps": GFS_STEPS, "model": "NOAA GFS", "provider": "NOAA/NCEP NOMADS",
            "getter": h.p66e._gfs_field, "display_bounds": h.GLOBAL_BOUNDS, "focus": h.FOCUS_GLOBAL,
            "intervals": [
                {"value": "auto", "label": "Automático (3 h)"}, {"value": "3", "label": "3 h"},
                {"value": "6", "label": "6 h"}, {"value": "12", "label": "12 h"}, {"value": "24", "label": "24 h"},
            ],
            "cadence": "publicación del visor cada 3 h hasta +384; sin interpolación",
        }
    return {
        "steps": ICON_STEPS, "model": "DWD ICON-EU", "provider": "Deutscher Wetterdienst (DWD) Open Data",
        "getter": h.p66e._icon_field, "display_bounds": h.ICON_CROP, "focus": h.FOCUS_ICON,
        "intervals": [
            {"value": "auto", "label": "Automático (1/3 h)"}, {"value": "1", "label": "1 h"},
            {"value": "3", "label": "3 h"}, {"value": "6", "label": "6 h"},
            {"value": "12", "label": "12 h"}, {"value": "24", "label": "24 h"},
        ],
        "cadence": "+0…+78 cada 1 h; +81…+120 cada 3 h",
    }


def validate_700_height(gh):
    finite = np.asarray(gh)[np.isfinite(gh)]
    if not finite.size:
        raise RuntimeError("700 hPa: sin altura geopotencial válida")
    mean = float(np.mean(finite))
    if not (1500.0 <= mean <= 4500.0):
        raise RuntimeError(f"700 hPa: altura geopotencial físicamente sospechosa, media={mean:.1f} m")
    return mean


def render_700(t_c, z_m, msl_hpa, bounds, out: Path):
...
def main():
    ...
    manifest = {
        "schema": 66, "phase": "66L", "status": "ok",
        "purpose": "700 hPa horizonte completo con diseño MapLibre validado; no producción",
        "model": cfg["model"], "data_provider": cfg["provider"], "run_utc": run_dt.isoformat(),
        "level_hpa": LEVEL, "generated_steps": list(steps), "horizon_hours": max(steps),
        "projection": "EPSG:3857", "display_bounds": cfg["display_bounds"], "source_cadence": cfg["cadence"],
        "publication_policy": "solo pasos oficiales disponibles; nunca interpolar ni inventar horas",
        "viewer": {"layout": "MapLibre 16:9 validado", "initial_view": cfg["focus"], "full_domain_button": True,
                   "scroll_wheel_zoom": False, "intervals": cfg["intervals"]},
        "style": {"level_hpa": LEVEL, "background": "geopotential_height_700hpa", "background_bands_m": 30,
                  "geopotential_isohypses_m": 30, "major_geopotential_isohypses_m": 60, "geopotential_labels": "dam",
                  "mean_sea_level_pressure_isobars_hpa": 4, "temperature_700hpa_contours_c": 4,
                  "format": "WEBP quality=92", "visual_source": "66I-B/66J/66K"},
        "maps": {},
    }
    ...
    for step in steps:
        ...
        try:
            ...
            mean_gh = validate_700_height(gh)
            out = base / f"700hpa_synoptic_mslp_{sk}.webp"
            size = render_700(tc, gh, p, bounds, out)
            manifest["maps"][sk] = {
                "status": "ok", "image": out.name, "bounds": bounds, "size": size,
                "temperature_range_c": h.p66e._finite_range(tc), "geopotential_height_range_m": h.p66e._finite_range(gh),
                "geopotential_height_mean_m": round(mean_gh, 2),
                "mean_sea_level_pressure_range_hpa": h.p66e._finite_range(p), "source_requests": sources,
            }
            successes += 1
    ...
    mp = base / "manifest-phase66l.json"
    mp.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps(manifest["summary"], ensure_ascii=False), flush=True)
    if manifest["status"] != "ok":
        raise RuntimeError(f"Fase 66L {MODEL} incompleta: " + " | ".join(failures[:8]))
