...
PUBLIC = ROOT / "public-icon48" / "icon-eu"
STEPS = tuple(range(16, 28))


def main():
    # Reutilizamos exactamente el motor de niveles/Jet validado en Fases 36, 46 y 47.
    p36.PUBLIC = PUBLIC
    p36.STEPS = STEPS
    p36.main()
    ...
    data["step_rule"] = "+16..+27 h cada 1 h"
    data["production_block"] = {"start_hour": 16, "end_hour": 27, "cadence_hours": 1}
    data["pressure_levels_hpa"] = list(p36.LEVELS)
    data["jet_levels_hpa"] = list(p36.JET_LEVELS)
    data["branding"] = credit_text(PUBLIC / "pressure" / "500hpa_temperature_geopotential" / "f016.webp")
    data["branding_position"] = "bottom-right"
    data["summary"]["map_files"] = len(STEPS) * (len(p36.LEVELS) + len(p36.JET_LEVELS))

    new = PUBLIC / "manifest-icon-eu48.json"
