...
def read_model(key: str):
    src = STAGING / key
    p = src / "manifest-phase66m.json"
    d = json.loads(p.read_text(encoding="utf-8"))
    if d.get("phase") != "66M" or d.get("status") != "ok" or d.get("level_hpa") != 925:
        raise RuntimeError(f"{key}: entrada 66M no válida")
    maps = {
...
def main():
    ...
    template = (
        base.TEMPLATE
        .replace("66I-B", "66M")
        .replace("__phase66ib", "__phase66m")
        .replace("500 hPa", "925 hPa")
        .replace("encuadre sin bordes del raster", "925 hPa completo · encuadre sin bordes")
    )
    ...
    report = {
        "phase": "66M",
        "status": "ok",
        "source_visual_phase": "66I-B/66J/66K/66L",
        "purpose": "925 hPa completo con MapLibre y selector temporal real",
    ...
    (OUT / "report-phase66m.json").write_text(
