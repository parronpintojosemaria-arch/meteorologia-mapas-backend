...
def read_model(key: str):
    src = STAGING / key
    p = src / "manifest-phase66s.json"
    d = json.loads(p.read_text(encoding="utf-8"))
    if d.get("phase") != "66S" or d.get("status") != "ok" or d.get("level_hpa") != 200:
        raise RuntimeError(f"{key}: entrada 66S no válida")
    maps = {
...
def main():
    ...
    template = template.replace("66I-B", "66S").replace("__phase66ib", "__phase66s")
    template = template.replace("500 hPa · visor premium", "Jet Stream 200 hPa · visor premium")
    template = template.replace(
        "encuadre sin bordes del raster",
        "Jet Stream 200 hPa completo · encuadre sin bordes",
    )
    template = template.replace("${M().model} · 500 hPa + PMSL", "${M().model} · Jet Stream 200 hPa")
    template = template.replace("Geopotencial 500 hPa", "Geopotencial 200 hPa")
    template = template.replace(
        '<span class="key"><i class="line p"></i>Presión nivel del mar</span>',
        '<span>Colores: viento a 200 hPa (km/h), visible desde 60 km/h</span>',
    )
    template = template.replace(
    ...
    report = {
        "phase": "66S",
        "status": "ok",
        "purpose": "Jet Stream 200 hPa completo con MapLibre y selector temporal real",
        "models": {
            k: {"maps": len(v["maps"]), "horizon": max(v["steps"])}
    ...
    (OUT / "report-phase66s.json").write_text(
