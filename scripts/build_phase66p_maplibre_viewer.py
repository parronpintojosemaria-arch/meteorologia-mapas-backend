...
def read_model(key: str):
    src = STAGING / key
    p = src / "manifest-phase66p.json"
    d = json.loads(p.read_text(encoding="utf-8"))
    if d.get("phase") != "66P" or d.get("status") != "ok" or d.get("level_hpa") != 200:
        raise RuntimeError(f"{key}: entrada 66P no válida")
    maps = {
        sk: {"image": r["image"], "bounds": r["bounds"], "size": r["size"]}
        for sk, r in d["maps"].items() if r.get("status") == "ok"
    }
    return {
        "model": d["model"], "provider": d["data_provider"], "run_utc": d["run_utc"], "level_hpa": 200,
...
def main():
    ...
    template = (
        base.TEMPLATE
        .replace("66I-B", "66P")
        .replace("__phase66ib", "__phase66p")
        .replace("500 hPa", "200 hPa")
        .replace("encuadre sin bordes del raster", "200 hPa completo · encuadre sin bordes")
    )
    ...
    report = {
        "phase": "66P", "status": "ok", "source_visual_phase": "66I-B/66J/66K/66L/66M/66N/66O",
        "purpose": "200 hPa completo con MapLibre y selector temporal real",
        "models": {k: {"maps": len(v["maps"]), "horizon": max(v["steps"])} for k, v in data.items()},
        "total_maps": sum(len(v["maps"]) for v in data.values()), "level_hpa": 200,
    ...
    (OUT / "report-phase66p.json").write_text(
