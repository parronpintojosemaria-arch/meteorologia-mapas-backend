...
PUBLIC = ROOT / "public-icon51" / "icon-eu"
STEPS = tuple(range(52, 64))
...
def main():
    ...
    data["step_rule"] = "+52..+63 h cada 1 h"
    data["production_block"] = {"start_hour": 52, "end_hour": 63, "cadence_hours": 1}
    data["pressure_levels_hpa"] = list(p36.LEVELS)
    data["jet_levels_hpa"] = list(p36.JET_LEVELS)
    data["branding"] = credit_text(PUBLIC / "pressure" / "500hpa_temperature_geopotential" / "f052.webp")
    data["branding_position"] = "bottom-right"
    data["summary"]["map_files"] = len(STEPS) * (len(p36.LEVELS) + len(p36.JET_LEVELS))

    new = PUBLIC / "manifest-icon-eu51.json"
