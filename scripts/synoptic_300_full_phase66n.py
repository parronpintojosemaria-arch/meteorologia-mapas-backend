...
if MODEL not in {"ecmwf", "gfs", "icon"}:
    raise SystemExit("Uso: synoptic_300_full_phase66n.py ecmwf|gfs|icon")

# Reutiliza el motor de descarga/proyección ya validado en 66H–66M.
...
# Escala específica de 300 hPa.
Z_BOUNDS = np.arange(6600.0, 11521.0, 60.0, dtype="float32")
TEMP_CONTOURS = np.arange(-92.0, -3.0, 4.0, dtype="float32")
...
def validate_300_height(gh):
    finite = np.asarray(gh)[np.isfinite(gh)]
    if not finite.size:
        raise RuntimeError("300 hPa: sin altura geopotencial válida")
    mean = float(np.mean(finite))
    if not (6500.0 <= mean <= 11500.0):
        raise RuntimeError(f"300 hPa: altura geopotencial físicamente sospechosa, media={mean:.1f} m")
    return mean


def render_300(t_c, z_m, msl_hpa, bounds, out: Path):
...
def main():
    ...
    manifest = {
        "schema": 66, "phase": "66N", "status": "ok",
        "purpose": "300 hPa horizonte completo con diseño MapLibre validado; no producción",
        ...
        "viewer": {
            "layout": "MapLibre 16:9 validado", "initial_view": cfg["focus"],
            "full_domain_button": True, "scroll_wheel_zoom": False, "intervals": cfg["intervals"],
        },
        "style": {
            "level_hpa": LEVEL, "background": "geopotential_height_300hpa",
            "background_bands_m": 60, "geopotential_isohypses_m": 60,
            "major_geopotential_isohypses_m": 120, "geopotential_labels": "dam",
            "mean_sea_level_pressure_isobars_hpa": 4, "temperature_300hpa_contours_c": 4,
            "format": "WEBP quality=92", "visual_source": "66I-B/66J/66K/66L/66M",
        },
        "maps": {},
    }
    ...
    for step in steps:
        ...
        try:
            ...
            mean_gh = validate_300_height(gh)
            out = base / f"300hpa_synoptic_mslp_{sk}.webp"
            size = render_300(tc, gh, p, bounds, out)
            manifest["maps"][sk] = {
    ...
    mp = base / "manifest-phase66n.json"
    mp.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps(manifest["summary"], ensure_ascii=False), flush=True)
    if manifest["status"] != "ok":
        raise RuntimeError(f"Fase 66N {MODEL} incompleta: " + " | ".join(failures[:8]))
