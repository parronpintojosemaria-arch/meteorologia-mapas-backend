...
def _validate(speed_kmh, gh):
    ...
    if not sf.size or not zf.size:
        raise RuntimeError("Jet 300 hPa: campos U/V/geopotencial sin datos válidos")
    smin, smax = float(sf.min()), float(sf.max())
    if smin < -0.01 or smax > 650.0 or smax < 60.0:
        raise RuntimeError(f"Jet 300 hPa: velocidad físicamente sospechosa min={smin:.1f} max={smax:.1f} km/h")
    zmean = float(zf.mean())
    if not (6500.0 <= zmean <= 11500.0):
        raise RuntimeError(f"Jet 300 hPa: altura geopotencial sospechosa media={zmean:.1f} m")
    return zmean
...
def _ecmwf_jet(run_dt, step):
    ...
    fields, sources = {}, []
    for param in ("u", "v", "z"):
        target = p.e4.RAW / f"p66q_ecmwf_{param}_{LEVEL}_{run_dt:%Y%m%d%H}_f{step:03d}.grib2"
...
def _select_run(max_step):
    ...
    for run_dt in _candidates():
        try:
            ...
            time.sleep(0.4)
    raise RuntimeError(
        f"No se encontró pasada {MODEL} completa de Jet 300 hPa hasta +{max_step} h. "
        + " | ".join(errors[-5:])
    )
...
def main():
    ...
    manifest = {
        ...
        "phase": "66Q",
        "status": "ok",
        "purpose": "Jet Stream 300 hPa horizonte completo con U/V oficiales; no producción",
        "model": cfg["model"],
        ...
        "style": {
            ...
            "visual_source": "66I-B + presión premium validada",
        },
        "maps": {},
    }
    ...
    for step in steps:
        ...
        try:
            ...
            out = base / f"jet_stream_300hpa_{sk}.webp"
    ...
    mp = base / "manifest-phase66q.json"
    mp.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps(manifest["summary"], ensure_ascii=False), flush=True)
    if manifest["status"] != "ok":
        raise RuntimeError(f"Fase 66Q {MODEL} incompleta: " + " | ".join(failures[:8]))
