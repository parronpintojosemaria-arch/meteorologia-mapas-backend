...
def read_model(key: str):
    src = STAGING / key
    p = src / "manifest-phase66j.json"
    d = json.loads(p.read_text(encoding="utf-8"))
    if d.get("phase") != "66J" or d.get("status") != "ok":
        raise RuntimeError(f"{key}: entrada 66J no válida")
    maps = {}
...
def main():
    ...
    template = (
        base.TEMPLATE
        .replace("66I-B", "66J")
        .replace("__phase66ib", "__phase66j")
        .replace("encuadre sin bordes del raster", "horizonte completo · encuadre sin bordes")
    )
    ...
    report = {
        "phase": "66J",
        "status": "ok",
        "source_visual_phase": "66I-B",
        "purpose": "500 hPa completo con MapLibre y selector temporal real",
    ...
    (OUT / "report-phase66j.json").write_text(
