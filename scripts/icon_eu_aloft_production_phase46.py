...
PUBLIC = ROOT / "public-icon46" / "icon-eu"
STEPS = (0, 1, 2, 3)


def main():
    # Reutilizamos exactamente el motor de niveles/Jet validado en Fase 36.
    p36.PUBLIC = PUBLIC
    p36.STEPS = STEPS
    p36.main()
    ...
    data["step_rule"] = "+0..+3 h cada 1 h"
    data["production_block"] = {"start_hour": 0, "end_hour": 3, "cadence_hours": 1}
    data["pressure_levels_hpa"] = list(p36.LEVELS)
    data["jet_levels_hpa"] = list(p36.JET_LEVELS)
    data["branding"] = credit_text(PUBLIC / "pressure" / "500hpa_temperature_geopotential" / "f000.webp")
    data["branding_position"] = "bottom-right"
    data["summary"]["map_files"] = len(STEPS) * (len(p36.LEVELS) + len(p36.JET_LEVELS))

    new = PUBLIC / "manifest-icon-eu46.json"
