...
with sync_playwright() as p:
    ...
    page.goto("http://127.0.0.1:8767/index.html", wait_until="domcontentloaded", timeout=45000)
    page.wait_for_function("window.__phase66jReady === true", timeout=45000)
    ...
    for model, (count, last_step) in EXPECTED.items():
        ...
        st = page.evaluate("window.__phase66jState()")
        assert st["overlayReady"] is True, st
        assert st["insideWeather"] is True, st
        assert st["step"] == last_step, st
        checks[model] = st

    page.locator("#model").select_option("ecmwf")
    page.wait_for_timeout(400)
    page.locator("#interval").select_option("6")
    page.wait_for_timeout(250)
    assert page.locator("#step option").count() == 61

    page.locator("#model").select_option("gfs")
    page.wait_for_timeout(400)
    page.locator("#interval").select_option("6")
    page.wait_for_timeout(250)
    assert page.locator("#step option").count() == 65

    page.locator("#model").select_option("icon")
    page.wait_for_timeout(400)
    page.locator("#interval").select_option("6")
    page.wait_for_timeout(250)
    assert page.locator("#step option").count() == 21

    page.screenshot(path=str(OUT / "screenshot-phase66j.png"), full_page=True)
    report = {
        "phase": "66J",
        "browser": "chromium",
    ...
    (OUT / "browser-smoke-phase66j.json").write_text(
...
print("66J browser smoke OK · horizontes finales 3/3 · intervalos reales OK")
