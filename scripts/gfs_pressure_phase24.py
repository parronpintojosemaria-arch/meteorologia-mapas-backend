...
def to_celsius(values, units):
    u = (units or "").lower()
    if units == "K" or "kelvin" in u or np.nanmean(values) > 100:
...
def render_composite(t_c, gh_m, level, bounds, out):
...
def lock_run():
    ...
    for run_dt in candidate_runs():
        try:
            retrieve_field(run_dt, 0, 925, "var_TMP", "gfs_tmp")
            retrieve_field(run_dt, 0, 925, "var_HGT", "gfs_hgt")
            return run_dt
        except Exception as exc:
            errors.append(f"{run_dt.isoformat()}: {exc}")
    raise RuntimeError("No se encontró una ejecución GFS completa para Fase 24. " + " | ".join(errors[-4:]))
...
def main():
    ...
    manifest = {
        ...
        "rendering": "temperature raster and geopotential-height contours share identical projected grid",
        "levels": {},
        "status": "ok",
    }
    ...
    for level in LEVELS:
        ...
        for step in STEPS:
            sk = f"f{step:03d}"
            try:
                tv, tu, tb, turls = retrieve_field(run_dt, step, level, "var_TMP", "gfs_tmp")
                zv, zu, zb, zurls = retrieve_field(run_dt, step, level, "var_HGT", "gfs_hgt")
                if tv.shape != zv.shape or not same_bounds(tb, zb):
                    raise RuntimeError("Las mallas GFS de temperatura y altura geopotencial no coinciden")
                tc = to_celsius(tv, tu)
                gh = to_height_m(zv, zu)
                out = PUBLIC / "gfs" / f"{level}hpa_temperature_geopotential" / f"f{step:03d}.webp"
    ...
    (PUBLIC / "manifest-gfs-pressure24.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps(manifest["summary"], ensure_ascii=False))
    if failures or successes != expected:
        raise RuntimeError(f"Fase 24 incompleta: {successes}/{expected} mapas correctos, {failures} fallos")
