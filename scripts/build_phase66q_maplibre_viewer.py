...
def read_model(key: str):
    src = STAGING / key
    p = src / "manifest-phase66q.json"
    d = json.loads(p.read_text(encoding="utf-8"))
    if d.get("phase") != "66Q" or d.get("status") != "ok" or d.get("level_hpa") != 300:
        raise RuntimeError(f"{key}: entrada 66Q no válida")
    maps = {
    ...
    return {
        ...
        "level_hpa": 300, "steps": d["generated_steps"], "display_bounds": d["display_bounds"],
...
def main():
    ...
    template = template.replace("66I-B", "66Q").replace("__phase66ib", "__phase66q")
    template = template.replace("500 hPa · visor premium", "Jet Stream 300 hPa · visor premium")
    template = template.replace("encuadre sin bordes del raster", "Jet Stream 300 hPa completo · encuadre sin bordes")
    template = template.replace("${M().model} · 500 hPa + PMSL", "${M().model} · Jet Stream 300 hPa")
    template = template.replace("Geopotencial 500 hPa", "Geopotencial 300 hPa")
    template = template.replace(
        '<span class="key"><i class="line p"></i>Presión nivel del mar</span>',
        '<span>Colores: viento a 300 hPa (km/h), visible desde 60 km/h</span>'
    )
    template = template.replace(
    ...
    report = {
        "phase": "66Q", "status": "ok",
        "purpose": "Jet Stream 300 hPa completo con MapLibre y selector temporal real",
    ...
    (OUT / "report-phase66q.json").write_text(
