...
def read_model(key: str):
    src = STAGING / key
    p = src / "manifest-phase66o.json"
    d = json.loads(p.read_text(encoding="utf-8"))
    if d.get("phase") != "66O" or d.get("status") != "ok" or d.get("level_hpa") != 250:
        raise RuntimeError(f"{key}: entrada 66O no válida")
    maps = {
...
def main():
    ...
    template = (
        base.TEMPLATE
        .replace("66I-B", "66O")
        .replace("__phase66ib", "__phase66o")
        .replace("500 hPa", "250 hPa")
        .replace("encuadre sin bordes del raster", "250 hPa completo · encuadre sin bordes")
    )
    ...
    report = {
        "phase": "66O",
        "status": "ok",
        "source_visual_phase": "66I-B/66J/66K/66L/66M/66N",
        "purpose": "250 hPa completo con MapLibre y selector temporal real",
    ...
    (OUT / "report-phase66o.json").write_text(
