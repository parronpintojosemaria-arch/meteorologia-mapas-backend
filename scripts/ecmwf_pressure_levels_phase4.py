...
LEVELS = [int(x) for x in os.getenv("PRESSURE_LEVELS", "850").split(",") if x.strip()]
...
def main():
    ...
    (PUBLIC / "manifest-pressure-levels.json").write_text(
        json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    print(json.dumps(manifest["summary"], ensure_ascii=False))

    if successes == 0:
        raise RuntimeError("No se pudo generar ningún mapa real de niveles de presión.")
