...
with sync_playwright() as p:
    ...
    page.goto("http://127.0.0.1:8775/index.html", wait_until="domcontentloaded", timeout=45000)
    page.wait_for_function("window.__phase66rReady === true", timeout=45000)
    assert "Jet Stream 250 hPa" in page.locator("h1").inner_text()
    ...
    for model, (count, last_step) in EXPECTED.items():
        ...
        st = page.evaluate("window.__phase66rState()")
    ...
    page.screenshot(path=str(OUT / "screenshot-phase66r.png"), full_page=True)
    report = {
        "phase": "66R", "level_hpa": 250, "variable": "jet_stream",
        "browser": "chromium", "full_horizon_checks": checks,
        "console_errors": errors, "http_errors": bad_http,
    }
    (OUT / "browser-smoke-phase66r.json").write_text(
...
print("66R browser smoke OK · Jet Stream 250 hPa · horizontes finales 3/3 · intervalos reales OK")
