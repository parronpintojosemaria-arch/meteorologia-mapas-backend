...
RAW_ROOT=ROOT/'.vnext-raw-aifs'; RAW_ROOT.mkdir(parents=True,exist_ok=True)
...
def retrieve(group,run_dt,step,levtype,params,target,levels=None,attempts=3):
 req={'stream':'oper','type':'fc','step':int(step),'levtype':levtype,'param':list(params),
      'date':int(run_dt.strftime('%Y%m%d')),'time':int(run_dt.strftime('%H'))}
 ...
 for source in SOURCE_ORDER[group]:
  for attempt in range(1,attempts+1):
   try:
    ...
    Client(source=source,model='aifs-single',resol='0p25',infer_stream_keyword=False).retrieve(**req,target=str(target))
...
def select_level(da,level):
 ...
 for name in da.coords:
  low=name.lower()
  if ('isobaric' in low or low in {'level','pressure'}) and (da.coords[name].size>1 or name in da.dims):
   coord=name;break
...
def project(vals,bounds,bbox,width,resampling=Resampling.cubic):
 if bounds['west']>bbox['west']+.2 or bounds['east']<bbox['east']-.2 or bounds['south']>bbox['south']+.2 or bounds['north']<bbox['north']-.2:
  raise RuntimeError(f'fuente {bounds} no cubre {bbox}')
...
def percent(v,u):
 f=v[np.isfinite(v)]
 return v*100 if f.size and float(np.nanmax(f))<=1.5 else v
def mm_accum(v,u):
...
def zheight(v,u):
 u=str(u).lower()
 return v/G0 if 'm**2' in u or 'm2' in u or 's**-2' in u or np.nanmean(v)>10000 else v

def surface_fields(run_dt,step):
 raw=RAW_ROOT/f'surface_{run_dt:%Y%m%d%H}_f{step:03d}.grib2'
 params=['2t','10u','10v','tcc','msl']+(['tp','sf','cp'] if step>0 else [])
 source,errs=retrieve('surface',run_dt,step,'sfc',params,raw)
 ds=open_grib(raw)
 aliases={'2t':{'2t','t2m'},'10u':{'10u','u10'},'10v':{'10v','v10'},'tcc':{'tcc'},'msl':{'msl','prmsl'},'tp':{'tp'},'sf':{'sf'},'cp':{'cp'}}
 out={}
 for k,names in aliases.items():
  if step==0 and k in {'tp','sf','cp'}:continue
...
def pressure_fields(run_dt,step,levels,group):
 ...
 source,errs=retrieve(group,run_dt,step,'pl',['t','z','u','v'],raw,levels=list(levels));ds=open_grib(raw);out={}
 for lev in levels:
...
def choose_cycle():
 ...
 for run in candidates():
  try:
   ...
   retrieve('prepare',run,360,'sfc',['2t','10u','10v','tcc','msl','tp','sf','cp'],s,attempts=1)
   ds=open_grib(s)
   for k in ('2t','10u','10v','tcc','msl','tp','sf','cp'):grid(find_da(ds,{k,'t2m' if k=='2t' else k}))
   s.unlink(missing_ok=True)
   p=RAW_ROOT/f'probe_p_{run:%Y%m%d%H}.grib2'
   retrieve('prepare',run,360,'pl',['t','z','u','v'],p,levels=[925,200],attempts=1);ds=open_grib(p)
   for k in ('t','z','u','v'):
 ...
 raise RuntimeError('No hay ciclo AIFS completo +360: '+' | '.join(errors[-8:]))
