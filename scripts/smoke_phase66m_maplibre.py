...
with sync_playwright() as p:
    ...
    page.goto("http://127.0.0.1:8770/index.html", wait_until="domcontentloaded", timeout=45000)
    page.wait_for_function("window.__phase66mReady === true", timeout=45000)
    assert "925 hPa" in page.locator("h1").inner_text()
    ...
    for model, (count, last_step) in EXPECTED.items():
        ...
        st = page.evaluate("window.__phase66mState()")
    ...
    page.screenshot(path=str(OUT / "screenshot-phase66m.png"), full_page=True)
    report = {
        "phase": "66M",
        "level_hpa": 925,
        "browser": "chromium",
    ...
    (OUT / "browser-smoke-phase66m.json").write_text(
...
print("66M browser smoke OK · 925 hPa · horizontes finales 3/3 · intervalos reales OK")
