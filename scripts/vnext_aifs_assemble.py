...
EXPECTED={'surface':848,'lower':732,'upper':732};TOTAL=sum(EXPECTED.values());STEPS=list(range(0,361,6))
def sha(path):
 h=hashlib.sha256()
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--root',default='vnext-full-merged/aifs');ap.add_argument('--summary-dir',default='vnext-full-summary/aifs');a=ap.parse_args();root=ROOT/a.root;summary=ROOT/a.summary_dir
 ...
 for i,row in enumerate(files,1):
  ...
  if i%250==0 or i==len(files):print(f'AIFS ASSEMBLE {i}/{len(files)}',flush=True)
 if dict(dc)!={'spain':1156,'europe':1156}:raise RuntimeError(f'dominios {dict(dc)}')
 for domain in ('spain','europe'):
  for step in (12,144,360):
 ...
 manifest={'schema':2,'architecture':'vnext-model-shard-current-json','model':'aifs','model_label':'ECMWF AIFS','data_provider':'ECMWF Open Data','cycle':cycle,'run_utc':run,'status':'ready','production_changed':False,'horizon_hours':360,'forecast_steps':STEPS,'cadence_policy':{'main':'6 h +0..+360','exact_6h_full_horizon':True,'never_invent_missing_hours':True,'unsupported_official_fields':['precipitation_rate','precipitation_type']},'domains':{'spain':{'render_width':2400,'view':'España'},'europe':{'render_width':2800,'view':'Europa'}},'quality':{'webp_quality':92,'render_dpi':180,'continuous_resampling':'cubic','no_invented_spatial_resolution':True,'vector_basemap_policy':'costas, fronteras y nombres se dibujan en el visor'},'product_counts':dict(sorted(pc.items())),'expected_count':TOTAL,'files':sorted(files,key=lambda x:(x['domain'],x['product'],x['step_hours'])),'semantic_policy':{'precipitation_total':'AIFS acumulado desde inicio de pasada','precipitation_convective':'AIFS acumulado convectivo oficial','snowfall_water_equivalent':'AIFS equivalente de agua acumulado','precipitation_rate':'no publicado: no se fabrica','precipitation_type':'no publicado: no se fabrica','fronts':'no se fabrican'}}
 (cycle_dir/'manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
 summary.mkdir(parents=True,exist_ok=True);mib=total/1024/1024;report={'phase':'vNext AIFS Single v2 full','status':'ok','production_changed':False,'cycle':cycle,'maps':TOTAL,'image_mib':round(mib,2),'single_pages_guard_mib':940,'fits_single_model_pages_repo':mib<=940,'domain_counts':dict(dc),'product_counts':dict(sorted(pc.items())),'product_mib':{k:round(v/1024/1024,2) for k,v in sorted(pb.items())}}
 (summary/'report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');shutil.copy2(cycle_dir/'manifest.json',summary/'manifest.json')
 samples=summary/'samples';samples.mkdir(exist_ok=True)
 wanted=[('spain','precipitation_total',12),('europe','precipitation_convective',12),('spain','wind_10m',144),('europe','temperature_850hpa',360),('spain','analysis_500hpa',144),('europe','jet_300hpa',360),('spain','mslp',144)]
