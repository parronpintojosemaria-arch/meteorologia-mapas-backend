...
def main():
    ...
    ap.add_argument('--root',default='vnext-full-merged/aifs')
    ap.add_argument('--summary-dir',default='vnext-full-summary/aifs')
    a=ap.parse_args()
    root=ROOT/a.root
    summary=ROOT/a.summary_dir
    ...
    for group,(p,f) in sorted(by.items()):
        if f.get('status')!='ready' or f.get('production_changed') is not False:
            raise RuntimeError(f'{group} inválido')
        if f.get('expected_count')!=EXPECTED[group] or len(f.get('files',[]))!=EXPECTED[group]:
            raise RuntimeError(f'conteo {group} inválido')
        files+=f['files']
        sources[group]=f.get('sources',{})
    ...
    for i,row in enumerate(files,1):
        ...
        pc[row['product']]+=1
        dc[row['domain']]+=1
        pb[row['product']]+=p.stat().st_size
        if i%250==0 or i==len(files):
            print(f'AIFS ASSEMBLE {i}/{len(files)}',flush=True)

    if dict(dc)!={'spain':1096,'europe':1096}:
        raise RuntimeError(f'dominios {dict(dc)}')

    for domain in ('spain','europe'):
        for step in (0,144,360):
            for prod in ('temperature_2m','analysis_850hpa','analysis_500hpa'):
                alpha_guard(cycle_dir/'images'/domain/prod/f'f{step:03d}.webp',f'{domain}/{prod}/f{step:03d}')
    ...
    manifest={
        ...
        'quality':{
            ...
            'vector_basemap_policy':'costas, fronteras y nombres se dibujan en el visor'
        },
        'product_counts':dict(sorted(pc.items())),
        'expected_count':TOTAL,
        'files':sorted(files,key=lambda x:(x['domain'],x['product'],x['step_hours'])),
        'semantic_policy':{
            'precipitation_total':'acumulado desde inicio de pasada',
            'snowfall_water_equivalent':'equivalente de agua acumulado',
            'precipitation_rate':'no publicado en este candidato; no se inventa',
            'precipitation_type':'no publicado en este candidato; no se inventa'
        }
    }
    ...
    report={
        ...
        'product_counts':dict(sorted(pc.items())),
        'product_mib':{k:round(v/1024/1024,2) for k,v in sorted(pb.items())}
    }
    ...
    wanted=[
        ('spain','precipitation_total',24),
        ('europe','precipitation_total',24),
        ('spain','wind_10m',144),
        ('europe','wind_10m',144),
        ('spain','temperature_850hpa',360),
        ('europe','temperature_850hpa',360),
        ('spain','analysis_500hpa',144),
        ('europe','analysis_500hpa',144),
        ('spain','jet_250hpa',360),
        ('europe','jet_250hpa',360),
        ('spain','mslp',144),
        ('europe','mslp',144)
    ]
    for domain,prod,step in wanted:
        shutil.copy2(cycle_dir/'images'/domain/prod/f'f{step:03d}.webp',samples/f'{domain}__{prod}__f{step:03d}.webp')
