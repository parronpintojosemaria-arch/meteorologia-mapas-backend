...
with sync_playwright() as p:
    ...
    page.goto("http://127.0.0.1:8771/index.html", wait_until="domcontentloaded", timeout=45000)
    page.wait_for_function("window.__phase66nReady === true", timeout=45000)
    assert "300 hPa" in page.locator("h1").inner_text()
    ...
    for model, (count, last_step) in EXPECTED.items():
        ...
        st = page.evaluate("window.__phase66nState()")
    ...
    page.screenshot(path=str(OUT / "screenshot-phase66n.png"), full_page=True)
    report = {
        "phase": "66N", "level_hpa": 300, "browser": "chromium",
        "full_horizon_checks": checks, "console_errors": errors, "http_errors": bad_http,
    }
    (OUT / "browser-smoke-phase66n.json").write_text(
...
print("66N browser smoke OK · 300 hPa · horizontes finales 3/3 · intervalos reales OK")
